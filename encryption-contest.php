...
require_once ( 'includes/ec-codes.php' );    // Class for translating right solution to code
require_once ( 'includes/ec-options.php' );  // Class for generating options page in admin site
require_once ( 'includes/ec-menu.php' );     // Class for generating page in main admin menu - place, where admin can create and manage tasks
...
class EncryptionContest {
  ...
  public function defaults() {
      ...
      $settings['rightSolutionPicture'] = plugin_dir_url( __FILE__  ) .'includes/smile.png';
      $settings['titleTag'] = '3';
  ...
  public function editTextForOutput($text) {
      ...
      $conversion_table = array ('ä'=>'a', 'ě'=>'e', 'š'=>'s', 'č'=>'c', 'ľ'=>'l', 'ĺ'=>'l', 'ř'=>'r', 'ž'=>'z', 'ý'=>'y', 'á'=>'a', 'í'=>'i', 'é'=>'e', 
                              'ú'=>'u', 'ů'=>'u', 'ü'=>'u', 'ű'=>'u', 'ó'=>'o', 'ö'=>'o', 'ő'=>'o', 'ô'=>'o', 'ť'=>'t', 'ŕ'=>'r', 'ď'=>'d', 'ň'=>'n', '. 
                              '=>'.', ','=>' ', '!'=>'', '?' => '', '-' => '', );
  ...
  public function showAnswerContent($current_task) {
      ...
      if ( $status == '' && $settings['published_task'] == $current_task )
          $content = '<textarea name="user_answer" id="user_answer" rows="4"></textarea>';          
  ...
  public function titleTag() {
      ...
      if (!isset($number))                 // Added feature in version 1.1, if number isn't defined in settings, it will use default number 3.
          $number = '3';
      
      $tag_open  = '<h'.$number.'>';
      $tag_close = '</h'.$number.'>'; 
  
      return array( $tag_open, $tag_close );
  }
  ...
  public function displayShortcode( $atts ) {
      ...
      if ( $current_task ) {
          ...
          $content .= $title_tag[0] . __('Assignment', 'encrypt') . $title_tag[1]; 
          ...
          $content .= $title_tag[0] .'<label for="user_answer">' . __('Decoded result', 'encrypt') . '</label>'. $title_tag[1];
          $content .= '<p>'. $classEc->showAnswerContent($current_task) .'</p>';
      
          $classEc->saveNotes();
          $content .= $title_tag[0] .'<label for="user_notes">' . __( 'Place for notes', 'encrypt') . '</label>'. $title_tag[1];
          ...
          if ( $settings['showUsers'] ) {
              $content .= $title_tag[0] . __('Successfully resolved', 'encrypt'). $title_tag[1];
